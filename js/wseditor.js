    ...
    var WYSIWYG = function() {
        ...
        return _createClass(WYSIWYG, [{
            ...
            value: function insertLink(e) {
                if (e.preventDefault(),
                !this.isFocusOnIncorrectTextArea()) {
                    var url = prompt(this.options.translations.enterurl + ":", "http://")
                      , description = document.getSelection();
                    null === url | "http://" === url || ("" === description.toString() && (description = prompt(this.options.translations.linkdescription + ":", this.options.translations.linkname)),
                    document.execCommand("insertHTML", !1, '<a href="' + url + '" target="_blank">' + description + "</a>"))
                }
            }
        }, {
            key: "insertPicture",
            ...
            value: function performAction(e) {
                if (e.preventDefault(),
                !this.isFocusOnIncorrectTextArea()) {
                    var $btn, action = $(e.target).closest("button").data("action");
            ...
            value: function setColor(e) {
                ...
                !this.isFocusOnIncorrectTextArea()) {
                    ...
                    document.execCommand("foreColor", !1, color),
                    $(e.target).closest(".mdb-wysiwyg-toolbar-options-list").removeClass("visible")
                }
            }
        }, {
            ...
            value: function toggleHTML(e) {
                ...
                this.isCodeShown ? ($source.html($source.text()).removeClass("show-raw"),
                this.isCodeShown = !1,
                $ref.removeClass("active")) : ($source.text($source.html()).addClass("show-raw"),
                this.isCodeShown = !0,
                $ref.addClass("active"))
            }
        }, {
